...
@implementation MCAlbumManager

+(MCAlbumManager*)manager{
    static MCAlbumManager* feedManager;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        feedManager=[[MCAlbumManager alloc] init];
    });
    return feedManager;
}
...
-(void)commentToUser:(AVUser*)toUser AtAlbum:(LCAlbum*)album content:(NSString*)content block:(AVBooleanResultBlock)block{
    MCComment* comment=[MCComment object];
